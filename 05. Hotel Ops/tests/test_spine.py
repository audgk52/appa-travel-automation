...
def test_path_b_happy_runs_full_spine(make_store):
    ...
    res = commit(store, StateStore(), prev)
...
def test_path_b_r1_gate_cannot_be_skipped(make_store):
    ...
    with pytest.raises(ValueError):
        commit(store, StateStore(), prev)                    # no R1 disposition → refused
    # Supplying disposition B (limited-check authorized) lets it proceed.
    res = commit(store, StateStore(), prev, grouping_disposition="B", limited_check_authorized=True)
    assert res.overall == "complete"
...
def test_path_b_needs_decision_blocks_commit_until_resolved(make_store):
    store, _ = make_store([record(name="James", record_id="rl-a", stay_id="STAY-1")])
    gated = _gated_on_payer(store)
    assert gated.status == "needs_decision"
    with pytest.raises(UnresolvedDecision):
        commit(store, StateStore(), gated)                   # unresolved → refused


def test_path_b_decision_resolved_allows_commit(make_store):
    store, _ = make_store([record(name="James", record_id="rl-a", stay_id="STAY-1")])
    res = commit(store, StateStore(), _gated_on_payer(store), decisions={"payer": "Production"})
    assert res.overall == "complete"


# --- Path A end-to-end -----------------------------------------------------------

def test_path_a_happy_runs_full_spine(make_store):
    store, _ = make_store([record(name="James", record_id="rl-a", stay_id="STAY-1",
                                  check_in="2026-06-10", check_out="2026-06-12", nights="2",
...
def test_path_a_r1_then_grouping_then_commit(make_store):
    ...
    assert commit(store, StateStore(), prev2).overall == "complete"
